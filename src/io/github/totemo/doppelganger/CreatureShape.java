...
public class CreatureShape {
     ...
     * @param triggerMaterial the Material of the final placed block that
     *        triggers creature spawning.
     * @param enabled if true, building this shape will summon a doppelganger;
     *        if false, the creature can only be summoned by command.
     */
    ...
    // ------------------------------------------------------------------------
    /**
     * Signify that the {@link CreatureType} signified by its name can be
     * summoned by this shape, with the specified weighted probability.
     *
     ...
     *
     * If more than one {@link CreatureType} is associated with this shape, a
     * type will be selected at random according to weighted probability.
     *
     ...
     * @return true if placing the specified block at the specified location
     *         would result in a complete creature shape.
     */
     ...
     * the creature shape in all horizontal directions (but not above or below
     * the bounding box).
     *
     * The purpose of this is to prevent the use of Doppelganger spawning as a
     * way to grief a pre-existing structure that was not built for the purpose
     * of spawning a Doppelganger.
     *
    ...
    // ------------------------------------------------------------------------
    /**
     * Compute the offsets of the blocks in the border around the shape
     * (including the head) which must be air in order for the creature to
     * spawn.
     */
    protected void computeBorder() {
        ...
        // Extend the bounded volume +/- one block in the X and Z dimensions
        // only.
        --minX;
        ++maxX;
        --minZ;
    ...
    protected boolean hasBodyOffset(int x, int y, int z) {
        if (x == 0 && y == 0 && z == 0) {
            // Offset (0,0,0) is the head, explicitly precluded by
            // loadFromSection().
            return true;
        }
     ...
     * be air. These are automatically computed when the configuration is
     * loaded.
     */
