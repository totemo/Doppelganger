...
public class CommandController implements CommandExecutor
{
  ...
  /**
   * Registers all command handlers and subcommand handlers in a class, matching
   * them with their corresponding commands and subcommands registered to the
   * specified plugin.
   * 
   * @param plugin The plugin whose commands will be considered for registration
   * @param handler An instance of the class whose methods will be considered
   *          for registration
   */
  ...
  /**
   * An annotation interface that may be attached to a method to designate it as
   * a command handler. When registering a handler with this class, only methods
   * marked with this annotation will be considered for command registration.
   * 
   * @author AmoebaMan
   */
  ...
  /**
   * An annotation interface that may be attached to a method to designate it as
   * a subcommand handler. When registering a handler with this class, only
   * methods marked with this annotation will be considered for subcommand
   * registration.
   * 
   * @author AmoebaMan
   */
  ...
  /**
   * This is the method that "officially" processes commands, but in reality it
   * will always delegate responsibility to the handlers and methods assigned to
   * the command or subcommand Beyond checking permissions, checking
   * player/console sending, and invoking handlers and methods, this method does
   * not actually act on the commands
   */
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
  {
      ...
      if (subCommand != null)
      {
        ...
        if (subHandler != null && subMethod != null)
        {
          ...
          for (int i = 1; i < args.length; i++)
            subArgs[i - 1] = args[i];
          /*
           * If the method requires a player and the subcommand wasn't sent by
           * one, don't continue
           */
          ...
          /*
           * If the method requires a console and the subcommand wasn't sent by
           * one, don't continue
           */
          ...
          /*
           * If a permission is attached to this subcommand and the sender
           * doens't have it, don't continue
           */
     ...
     * this point Get the handler and method attached to this command
     */
    ...
    {
      /*
       * If the method requires a player and the command wasn't sent by one,
       * don't continue
       */
      ...
      /*
       * If the method requires a console and the command wasn't sent by one,
       * don't continue
       */
